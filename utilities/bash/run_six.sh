...
function dot_bsub(){
    ...
    multipleTrials "tmpLines=\"`bsub -q $lsfq -o $RundirFullPath/$Runnam.log $RundirFullPath/$Runnam.job 2>&1`\" ; taskno=\"`echo \"${tmpLines}\" | grep submitted | cut -d\< -f2 | cut -d\> -f1`\"" "[ -n \"\${taskno}\" ]" "Problem at bsub"
    let __lerr+=$?
...
while getopts  ":hgo:sctakfvBlSCMd:p:R:" opt ; do
    case $opt in
	a)
	    # do everything
	    lgenerate=true
