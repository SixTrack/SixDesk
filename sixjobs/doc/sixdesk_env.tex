...
the}\texttt{sixjobs/work} directory.  The \texttt{taskids} file contains one line for
each case. Each line contains the JobName (which can be mapped to the case
directory) and the associated LSF job\_ID(s) or BOINC ID(s).  If \texttt{run\_six}
...
simpler recovery is available with the \texttt{run\_missing\_jobs} command.
\emph{It is essential to wait for existing LSF jobs to complete, or to cancel
them, and do at least two \texttt{run\_status} commands before invoking {\tt
